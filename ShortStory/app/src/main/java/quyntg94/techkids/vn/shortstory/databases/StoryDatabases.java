package quyntg94.techkids.vn.shortstory.databases;
...
public class StoryDatabases extends SQLiteAssetHelper {
    ...
    public StoryDatabases(Context context) {
    ...
    public List<Story> loadAllStories(){
        ...
        //query ==> cursor
        Cursor cursor = sqLiteDatabase.query("tbl_story", STORY_ALL_COLUMNS, null, null, null, null, null);
    ...
    public void updateLastChapter(Story story, int lastChapterNo){
        SQLiteDatabase db = getReadableDatabase();

        ContentValues contentValues = new ContentValues();
        contentValues.put(STORY_LAST_CHAPTER_NO, lastChapterNo);
        db.update("tbl_story", contentValues, "id=?", new String[]{
           String.valueOf(story.getId())
        });
        db.close();
        story.setLastChapterNo(lastChapterNo);
